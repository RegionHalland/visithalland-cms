<div class="w-full sm:w-6/12 lg:w-5/12 xl:w-4/12 px-3 mb-6">
    <img class="mb-2" src="@asset('images/logo-horizontal.svg')" />
    <p class="font-fira italic text-sm text-grey-light mb-4">{{ bloginfo('description') }}</p>
</div>

<div class="w-full sm:w-6/12 lg:w-7/12 xl:w-8/12 flex px-3 mb-6 sm:justify-end items-start">
    <a href="mailto:info@visithalland.com" target="_blank" class="focus rounded-full link-reset inline-block">
        <div class="h-8 w-8 bg-blue-light rounded-full flex items-center justify-center text-white">
            <svg class="icon fill-current">
                <use xlink:href="#mail-icon"/>
            </svg>
        </div>
    </a>
    <a href="https://www.facebook.com/visithalland/" target="_blank" class="focus rounded-full link-reset inline-block ml-3">
        <div class="h-8 w-8 bg-blue-light rounded-full flex items-center justify-center text-white">
            <svg class="icon fill-current">
                <use xlink:href="#facebook-icon"/>
            </svg>
        </div>
    </a>
    <a href="https://www.instagram.com/visithalland/" target="_blank" class="focus rounded-full link-reset inline-block ml-3">
        <div class="h-8 w-8 bg-blue-light rounded-full flex items-center justify-center text-black">
            <svg class="icon fill-current">
                <use xlink:href="#instagram-icon"/>
            </svg>
        </div>
    </a>
</div>

