...
<div class="flex flex-wrap w-full px-3">
    ...
    <div class=" w-full sm:w-6/12 lg:w-3/12 mb-8">
        ...
        @if(is_array($navbar_items))
            @foreach($navbar_items as $primary_navigation_item)
                @if(!is_array($primary_navigation_item->children))
                    <a href="{{ $primary_navigation_item->url }}" class="font-rift font-bold text-grey opacity-75 hover:opacity-100 focus:opacity-100 mb-3 block">{!! $primary_navigation_item->title !!}</a>
                @endif
            @endforeach
        @endif
    </div>
    ...
    <div class=" w-full sm:w-4/12 lg:w-3/12 mb-8">
        ...
        @if(is_array(App::secondaryMenuItems()))
            @foreach(App::secondaryMenuItems() as $secondary_navigation_item)
                <a class="font-rift text-grey opacity-75 hover:opacity-100 focus:opacity-100 font-bold mb-3 block" href="{{ $secondary_navigation_item->url }}">{!! $secondary_navigation_item->title !!}</a>
            @endforeach
        @endif
        @if(is_array($non_active_langs))
            @foreach ($non_active_langs as $key => $lang)
                <a class="font-rift text-grey opacity-75 hover:opacity-100 focus:opacity-100 font-bold mb-3 block" href="{{ $lang["url"] }}">{{ $lang["native_name"] }}</a>
            @endforeach
        @endif
    </div>
</div>